...
**处置：已隐藏主题切换入口**（`ThemeSettingsFragment`），本版本只能改配色、不能改主题。

曾考虑由代码把 guyii 的键盘注入所有主题（一处定义对所有主题生效，包括用户自行添加的）。
最终没有采用 —— 这个输入法的全部价值都建立在自己那套键盘上，支持换主题等于支持把它换掉，
而换掉之后键盘看起来就是坏的、且没有任何线索说明原因。配色保留，37 套都安全。
