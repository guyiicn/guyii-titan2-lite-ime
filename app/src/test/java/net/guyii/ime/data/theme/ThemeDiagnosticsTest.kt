...
import net.guyii.ime.data.theme.ThemeDiagnostics.Code
import net.guyii.ime.data.theme.ThemeDiagnostics.Severity
import net.guyii.ime.data.theme.model.GeneralStyle
import net.guyii.ime.ime.keyboard.KeyCode
import net.guyii.ime.util.yaml.Yaml
    ...
    BehaviorSpec({
        ...
        Given("the key vocabularies") {
            Then("GeneralStyle.KNOWN_KEYS matches the keys decode reads") {
                val literals =
                    Regex("""node\["([a-z_0-9]+)"]""")
                        .findAll(File("src/main/java/net/guyii/ime/data/theme/model/GeneralStyle.kt").readText())
            ...
            Then("Theme.TOP_LEVEL_KEYS matches the keys decode reads") {
                val literals =
                    Regex("""node\["([a-z_0-9]+)"]""")
                        .findAll(File("src/main/java/net/guyii/ime/data/theme/Theme.kt").readText())
                        .map { it.groupValues[1] }
                        .toSet()
