...
- **屏幕功能行** —— 实体键盘缺的 `Esc / Tab / Ctrl / ↑↓←→` 和复制粘贴，外加一页四行符号；
  `▸` 走三档：功能行 → 四行符号页 → **vi 组合键页** → 回功能行。
  长按 `▸` 直达完整软键盘（实体键盘损坏时的退路）。每一档都有「功能行」键一键回到第一栏
...
屏幕只占一行。实体键盘缺的那几个键常驻在这里 —— 这是在 ssh 到自己机器上的 Termux 里实拍的：

![终端里的功能行](docs/ui/term-function-row.png)
...
![终端里的四行符号页](docs/ui/term-symbols.png)

**终端里也能用实体键盘打中文。** 终端把自己申报成「没有可编辑内容」，
多数输入法到这里就只转发原始按键了；这一版按有没有编辑器持有焦点来判断，
所以候选照常出来，而 `Ctrl-C`、方向键、退格仍然归终端：
...
只放三类东西 —— **多键序列**（`dd` `ciw` `gg`）、**Ctrl 组合**（`C-u` `C-d` `C-r`）、
**手机上难打的符号**（`$` `^` `%` `:` `/`）。单个字母（`h j k l`）字母键盘上本来就有，
放进来是浪费格子；`i a o v V u x w b p P` 留着是因为进出模式太高频。

这一页的每个键都走 `commit:` 或 `text:`，**没有一个是普通字符键** —— 普通字符键会喂给
rime，中文模式下按 `i` 就成了拼音的开头而不是进插入模式。

长按 `▸` 是一整块软键盘 —— 实体键盘万一坏了的退路。编辑功能都在长按上
...
> 一秒内完成，不碰词库。真正要盯的是 `round #`：那是词库编译，出现就说明产物失效了。
>
> **等待时间要够。** 词库重建在首次输入时才触发，`melt_eng` 一个就要 78 秒。
> 装完只等十几秒就 grep，会得到「零重建」的假象 —— 这个错误犯过一次，
> 让一版带着错误时间戳的产物发了出去。至少等 150 秒再判定。
...
- **主题切换入口已隐藏**（`ThemeSettingsFragment`）。只有本项目自己的主题定义了功能行、
  四行符号页和完整键盘退路，换到任何其它主题都会把它们无声地换成一块普通 QWERTY。
  配色不受影响 —— 换配色只是 `refreshColors()` 重新上色，不重建键盘，37 套都能安全使用
- **功能行必须叫 `rime_ice`** —— `smartMatchKeyboard()` 会优先选用与方案 id 同名的键盘，
  改名就不会被自动选中
- **`guyii_full` 是唯一被记住的键盘**。其余键盘每次聚焦都重新匹配（见上条 `.default`），
  因为要能从"Rime 未就绪时贴错键盘"里自愈；但完整软键盘是实体键盘坏掉时的退路，
  必须活过焦点切换，所以 `switchKeyboard()` 单独为它置了一个粘滞标志。
...
- redroid（x86_64 Android 容器）可验证键盘路由、候选渲染、部署行为；
  **但它没有触摸板，飞字只能在真机上验**
- **[`docs/test-checklist.md`](docs/test-checklist.md)** 是**每次发版照着执行**的测试清单：
  92 个测试点（19 项必须真机人工），标注了每项怎么测、预期是什么、能不能自动化，
  开头有一节「快速冒烟测试」供小版本使用，末尾是已知的「设置与本项目设计冲突」回归项。
  改代码之后要跑的是这一份
- **[`docs/test-report.md`](docs/test-report.md)** 是**某一个版本的实测结果存档**（当前为 3.4.0）：
  含截图、失败项根因、以及与本项目设计冲突的设置清单。它记录「那次测出来是什么」，
  不是「该测什么」
